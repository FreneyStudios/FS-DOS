...
__attribute__ ((section ("kernel_entry"))) void kernel_main(void) {
    ...
    while (!user_gfx_info->fg_color) {
        if (gfx_mode->bits_per_pixel > 8) {
            // 24-bit color: White foreground, Black background
            printf("\eFG%xBG%x;", convert_color(0xFFFFFF), convert_color(0x000000));
            user_gfx_info->fg_color = convert_color(0xFFFFFF);  // White foreground
            user_gfx_info->bg_color = convert_color(0x000000);  // Black background
        } else {
            // VGA palette: White foreground, Black background
            printf("\eFG%xBG%x;", convert_color(0x0F), convert_color(0x00));
            user_gfx_info->fg_color = convert_color(0x0F);  // White foreground in VGA
            user_gfx_info->bg_color = convert_color(0x00);  // Black background in VGA
        }
    }
